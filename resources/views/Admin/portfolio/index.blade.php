                            ...
                            <tbody>
                                @foreach ($portfolios as $portfolio)
                                    <tr>
                                        ...
                                        <td> <img src="{{ asset('uploads/portfolio/'.$portfolio->image) }}" alt="" width="200" height="100"></td>
                                        <td>
                                            <table>
                                                @if ($portfolio->etat == 0)
                                                    <tr>
                                                        <a href="{{ route('portfolio.active', $portfolio->id) }}"
                                                            title="Activer le portfolio"><i
                                                                class="fa fa-thumbs-down fa-2x fa-regular font-danger "></i>
                                                        </a>
                                                    </tr>
                                                @else
                                                    <tr><a href="{{ route('portfolio.active', $portfolio->id) }}"
                                                            title="Desactiver le portfolio le portfolio"><i
                                                            class="fa fa-thumbs-up fa-2x fa-regular font-danger "></i></a>
                                                    </tr>
                                                @endif

                                                <tr>
                                                    <tr><a href="{{ route('portfolio.edit', $portfolio->id) }}"> <i
                                                        class="fa fa-edit me-2 font-success fa-2x  "></i> </a>
                                            </tr>
                                                </tr>
                                                <tr><a href="{{ route('portfolio.show', $portfolio->id) }}"> <i
                                                            class="fa fa-eye fa-2x "></i> </a></tr>

                                                <tr>
                                                    <form action="{{ route('portfolio.destroy', $portfolio->id) }}"
                                                        id="destroy{{ $portfolio->id }}" method="POST">
                                                        @csrf
                                                        @method('DELETE')
                                                        <a href="#" onclick="event.preventDefault(); if(confirm('Êtes-vous sûr de vouloir supprimer ce portfolio ?')) { document.getElementById('destroy{{ $portfolio->id }}').submit(); }">
